...
*   **`vavavult::file`**
    ...
    *   `mod.rs`: Defines the `FileEntry` struct, which represents a file's metadata within the vault. Path listing types live in `vavavult::vault`: `DirectoryEntry` for directories, `FilePathEntry` for file path mappings, and `ListPathEntry` for mixed non-recursive list results.
