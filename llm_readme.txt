...
*   **`vavavult::vault`**
    ...
    *   **Database schema:** New vaults separate file content entities from path mappings. The `files` table stores encrypted content identity and keys only; `directories` stores the directory tree with a root record (`id = 1`); `file_entries` stores filename-to-file mappings for many-to-one hardlink-style references. `tags` now references `file_entries.id`, so the same file content can have different tags at different vault paths. `FileEntry` represents file content metadata and no longer stores a path or tags; callers that need display paths can use `Vault::list_paths_by_hash()` or the path-oriented listing APIs. `FilePathEntry` lives in `vavavult::file` and contains the vault path, encrypted hash, and path-local tags. `Vault::list_by_path()` returns `ListPathEntry`, wrapping `DirectoryEntry` (directory path, parent path, direct child counts) and `FilePathEntry`. `Vault::find_by_path()`, `Vault::find_by_paths()`, `Vault::find_by_tag()`, and `Vault::find_by_keyword()` are path-oriented and return `QueryPathResult` or `FilePathEntry` values; hash/content lookups return `QueryFileResult`. `Vault::list_all_recursive()` returns recursive `FilePathEntry` path mappings. Tag mutation APIs accept `&VaultPath` rather than `&VaultHash`. `Vault::get_file_count()` counts current directory file mappings (`file_entries`), while `Vault::get_storage_file_count()` counts actual stored file entities (`files`). Add operations now create missing directory records and insert `file_entries` mappings; duplicate original content reuses the existing file entity by default. Add convenience methods and Stage 3 commit accept an `Option<bool>` duplicate-content policy: `None` and `Some(true)` preserve automatic merge behavior, while `Some(false)` rejects duplicate original hashes with `AddFileError::DuplicateOriginalContent`. Hash-based remove operations (`remove_file` and `force_remove_file`) delete the file entity and all of its path mappings; path-based removal (`remove_file_by_path`) unlinks only the selected path and deletes the physical payload only after the final mapping is gone. Path operations live in `vavavult::vault::path_ops`: move operations are unified as `Vault::move_path(source_path, target_path)` and accept only `VaultPath` values, and in-place rename is exposed as `Vault::rename_path_inplace(source_path, new_name)` for both files and directories. `Vault::copy_file_path(source_path, target_path)` creates a new file path referencing the same file entity and copies source path-local tags; `Vault::create_path_from_hash(hash, target_path)` creates a new file path from an existing encrypted hash without tags; `Vault::create_empty_path(path)` creates an empty directory path and missing parent directories. File sources update `file_entries.directory_id`/`name` only; directory sources update the directory node so the whole subtree moves or renames without rewriting encrypted file content. Metadata operations are DB-first: `find`, `list`, keyword/tag search, tag mutation, path move/copy, and metadata-only removal trust the SQLite path/content records and do not fail just because the encrypted payload is missing from `data/`. Content operations keep the storage boundary strict: `extract`, stage-2 `rekey`, `verify_file_integrity`, and `fix` are responsible for detecting missing or corrupt payloads. File/directory path invariants are enforced by shared directory/file-entry creation helpers, so the DB cannot contain both `/a` as a file mapping and `/a/` as a directory node. Hash/path removal cleans database records first and treats missing physical payloads like `force_remove_file` does; unexpected storage loss should be repaired or cleaned by the `fix` toolchain. Reopened SQLite connections enable `PRAGMA foreign_keys = ON` so cascades such as `file_entries` → `tags` remain active after `open_vault`.
    ...
    *     3. **Commit** (`commit_addition_tasks(files, allow_duplicate_files)`): Commits encrypted files to DB (requires `&mut self`). The second parameter is `Option<bool>` where `None` defaults to allowing duplicate original content and automatically merging it into existing file entities; `Some(false)` rejects duplicate original hashes. In one batch, repeated original hashes are merged by inserting one `files` row and multiple `file_entries` mappings.
...
*   **`vavavult::file`**
    ...
    *   `mod.rs`: Defines `FileEntry` for content-level metadata and `FilePathEntry` for path mappings with path-local tags. `DirectoryEntry` and `ListPathEntry` remain in `vavavult::vault` for directory and mixed non-recursive list results.
