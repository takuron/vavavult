...
*   **`vavavult::vault`**
    ...
    *     3. **Commit** (`commit_addition_tasks` / `commit_addition_tasks_with_duplicate_control`): Commits encrypted files to DB (requires `&mut self`). The duplicate-control variant accepts `Option<bool>` where `None` defaults to allowing duplicate original content and automatically merging it into existing file entities; `Some(false)` rejects duplicate original hashes.
