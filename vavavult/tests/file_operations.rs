...
fn test_add_file_and_extract_file_cycle() {
    ...
    assert_eq!(fs::read_to_string(&extract_path).unwrap(), "Hello V2 Integrity");
}
...
fn test_add_file_paths_and_errors() {
    ...
    assert!(matches!(vault.add_file(&file2_path, &path1).unwrap_err(), AddFileError::DuplicateFileName(_)));

    // 错误 2: 尝试添加内容完全相同的文件 (即使路径不同)
    let path2 = VaultPath::from("/file1_copy.txt");
    assert!(matches!(vault.add_file(&file1_path, &path2).unwrap_err(), AddFileError::DuplicateOriginalContent(_, _)));
}
...
fn test_move_and_rename_file() {
    ...
    let hash = vault.add_file(&file_path, &VaultPath::from("/dir1/move.txt")).unwrap();
    ...
    assert!(matches!(vault.find_by_path(&VaultPath::from("/dir2/renamed.txt")).unwrap(), QueryResult::Found(_)));
}
...
fn test_remove_file() {
    ...
    let hash = vault.add_file(&file_path, &VaultPath::from("/del.txt")).unwrap();
...
fn test_large_file_integrity() {
    ...
    // 添加到 Vault 并重新提取
    let hash = vault.add_file(&source_path, &VaultPath::from("/large.bin")).unwrap();
...
fn test_batch_queries_and_search() {
    ...
    // 1. 批量哈希查询
    let res = vault.find_by_hashes(&[hash_a.clone(), hash_c.clone()]).unwrap();
    assert_eq!(res.len(), 2);
...
fn test_parallel_add_and_extract() {
    ...
    let tasks: Vec<(ExtractionTask, std::path::PathBuf)> = {
        ...
        all_files.iter().enumerate().map(|(i, f)| {
            (v.prepare_extraction_task(&f.sha256sum).unwrap(), extract_dir.join(format!("out_{}.txt", i)))
        }).collect()
    };
